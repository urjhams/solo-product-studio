...
6. Convert vague quality language into concrete checks. Every verification item must say what to check, where evidence comes from, and its current status.
...
**Prototype mode short form.** All six brief sections are still required, but Verification is the click-through path plus the single core-logic check, step 8 may end as `self_review_only` without blocking handoff, and the brief must state that the output is a throwaway prototype not to be extended without re-running mode selection. See `references/prototype-mode.md`.
